  ...
  const filteredPorts = useMemo(
    () =>
      portRecords.filter((port) => {
        const matchesKeyword = `${port.name}${port.device}${port.protocol}`.toLowerCase().includes(keyword.trim().toLowerCase());
        ...
        panes={[
          <section className="port-panel port-panel--left" key="left">
            <div className="port-search">
              <Input.Search allowClear placeholder="搜索端口、设备或协议" value={keyword} onChange={setKeyword} />
              <Select
                value={status}
                onChange={setStatus}
                options={[
