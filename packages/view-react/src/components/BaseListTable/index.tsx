...
>(({ className, onReady, option, style, ...restProps }, forwardedRef) => {
  ...
  const mergedOption = useMemo<ListTableConstructorOptions>(() => {
    const emptyTip =
      typeof option?.emptyTip === 'object' ? { ...baseOptions.emptyTip, ...option.emptyTip } : (option?.emptyTip ?? baseOptions.emptyTip);
  ...
  return (
    <div ref={containerRef} className={['base-list-table', className].filter(Boolean).join(' ')} style={style}>
      <ListTable {...restProps} ref={combinedRef} option={mergedOption} width="100%" height="100%" onReady={handleReady} />
    </div>
  );
});
